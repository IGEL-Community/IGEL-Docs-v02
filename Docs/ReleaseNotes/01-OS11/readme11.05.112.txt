...
Firmware version 11.05.112  
Release date 2021-03-10  
Last update of this document 2021-03-10  
...
+-------------------------------------------+----------------------------------+
| Kernel                                    | 5.9.16 #mainline-lxos-g1615272302 |
+-------------------------------------------+----------------------------------+
...
Release Notes 11.05.112 (Based On 11.05.110)
--------------------------------------------------------------------------------
...
* Available Citrix Workspace apps in this release: 21.03(default), 20.12 and
  19.12
* Added new Citrix feature for enhanced experieance of multi monitor scenarios.
...
+------------+-----------------------------------------------------------------+
|Parameter   |`Enhanced Experieance of Multi-Monitor scenarios`                |
+------------+-----------------------------------------------------------------+
...
* Fixed problem with Nexus Personal smart card in Gemalto IDBridge CT30 reader
  via Citrix and RDP
...
* Fixed issue where the Demo License browser crashes after EULA.
...
* All parameters of the citrix setlog program are now available in our setup
  under ica.logging.setlog. So the logging can be set better and permanent.  
  We also changed the structure of the parameters to show the inheritance more
  clearly.  
  As an example:  
  All parameters under all_ inherit from the parameter all.  
  All parameters under vd_ inherit from the parameter vd.  
  If the parameter vd itself is set to inherit, the parameters under vd_ inherit
  from all.
...
* On devices with 2 GB of flash storage it could happen that there is not enough space
  ...
  Please visit https://kb.igel.com/igelos-11.04/en/error-not-enough-space-on-local-drive-when-updating-to-igel-os-11-04-or-higher-32870765.html
  for a possible solution and additional information.
...
* After disconnect of an RDP based session, the Horizon main window which
  contains the server or sessions overview cannot be resized anymore.
* Copying Text from Horizon Blast sessions isn't possible
* The on-screen keyboard in Horizon appliance mode doesn't work correctly with
  local logon.  
  You have to switch off local logon and switch on this two keys in the IGEL
  registry:  
  userinterface.softkeyboard.autoshow  
  userinterface.softkeyboard.autohide
* Zoom VDI Media Plugin makes Horizon Client crash upon connection to the remote desktop
  in cases when TCSetup is running at the same time.
...
* TP-Link Archer T2UH WiFi adapters does not work after system
  suspend/resume. Workaround: Disable system suspend at IGEL Setup > System >
...
* Hyper-V (Generation 2) needs a lot of memory (RAM). The machine needs a sufficient
...
* Audio jack detection on Advantec POC-W243L does not work. Therefore, sound output goes
  through a possibly connected headset and also the internal speakers.
* IGEL UD2 (D220) fails to restore the volume level of the speaker when the device
  used firmware version 11.01.110 before.

### Hardware

* Some newer Delock 62599 active DisplayPort to DVI (4k) adapters only work
...
* Update from memory stick requires network online state (at least when multiple
  update stages are involved)
* Unreliable messages in user dialog for applying settings during boot. Could occure
  when new settings were fetched from the UMS.
