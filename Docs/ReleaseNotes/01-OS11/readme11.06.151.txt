...
Firmware version 11.06.151  
Release date 2021-11-17  
Last update of this document 2021-11-17  
...
+-------------------------------------------+----------------------------------+
| Kernel                                    | 5.12.19 #mainline-lxos-g1632904604 |
+-------------------------------------------+----------------------------------+
...
+--------------------------------------------+--------+------------------+
| Citrix StoreFront                          | 308.5M | Included         |
...
+--------------------------------------------+--------+------------------+
| Citrix Appliance                           |  76.8M | Included         |
...
+--------------------------------------------+--------+------------------+
| Fluendo Browser Codec Plugins              |   1.5M | Included         |
+--------------------------------------------+--------+------------------+
...
use UMS 6.08.110 or later and updating the corresponding profiles.
...
Release Notes 11.06.151 (Based On 11.06.150)
--------------------------------------------------------------------------------
...
* With Citrix Workspace app versions 21.04.0 and 21.06.0, Smartcard
  authentication fails to forward the PIN correctly into the session for login.
  Instead the user has to input the PIN a second time inside the session. The
...
* Client drive mapping and USB redirection for storage devices can be switched
  ...
  unclean unmount.  
  As a work-around you could add the IDs of these Card readers to IGEL USB
  access rules and deny them, so they are not mounted in IGEL OS.
...
* Certain modern web portals used for cloud login may fail to display because of
  incompatibility with the current firefox 78 ESR browser.
...
* If the monitor is set to the right most one conky will not be correctly
  positioned.  
* The horizontal offset should be set to the width of the monitor (e.g. if the
  right most monitor has a width of 1920, the offset should be set to 1920) to
...
* Browser Appliance mode can fail when the Web URL contains special control
  characters like ampersands (the & character).  
  You are on the safe side if you add quotes at the beginning and the end of
  such a URL. E.g.:  
...
* AIT feature with IGEL Starter License is only supported by UMS version
  6.05.100 or newer.
* If a parameter value is stored in a template parameter,  the "Apply changes"
  dialog is displayed on each boot while receiving remote settings. A possible
  workaround: Deactivate the "Apply changes" dialog with parameter:

+------------+-----------------------------------------------------------------+
|IGEL Setup  |System > Remote management                                       |
+------------+-----------------------------------------------------------------+
...
* Display the Energy-Star Logo in Setup Tool when the system requires it.
...
* Integrated **Citrix Workspace app 21.09**  
  Available Citrix Workspace Apps in this release: 21.09 (default), 21.04, and
  20.10
* New features:  
...
* Dynamically mounted shares are given to Horizon session on startup as shared
  folders.
...
* Removed registry key "imprivata.gain_permission". Since OneSign 6.3 is End-Of-
  Life by July 31 2021, we gain permission by default now.
...
* With the CWA 20.12 Citrix has revised the client drive mapping, with which we
  can no longer preset the right of the drives. Since then, the user can assign
...
* Compositor parameter will not be forced off anymore if opacity is set to 255
  for conky
...
* Fixed an issue where the taskbar was displayed incorrectly in some special
  configuration cases.

### Audio

* New Parameter for pulseaudio. Switch headset microphone suspend mode on/off:
...
* External drives mounted already before connection do not appear in the remote
  desktop.  
  Workaround: map the directory /media as a drive.  
  The external devices will then show up inside the media drive.
* After disconnect of an RDP based session, the Horizon main window which
  contains the server or sessions overview, cannot be resized anymore.
