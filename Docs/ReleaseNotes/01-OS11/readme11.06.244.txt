...
Firmware version 11.06.244  
Release date 2022-02-01  
Last update of this document 2022-02-01  
...
Release Notes 11.06.244 (Based On 11.06.240)
--------------------------------------------------------------------------------
...
+-------------------------------------------+----------------------------------+
| Cisco Webex VDI plugin                    | 41.12.0.20899                    |
+-------------------------------------------+----------------------------------+
| Cisco Webex Meetings VDI plugin           | 41.10.9.15                       |
+-------------------------------------------+----------------------------------+
| Cisco Webex Meetings VDI plugin           | 41.12.6.12                       |
+-------------------------------------------+----------------------------------+
| Cisco Webex Meetings VDI plugin           | 42.1.1.20                        |
+-------------------------------------------+----------------------------------+
...
+-------------------------------------------+----------------------------------+
| Olympus driver for dictation              | 4.0.1                            |
+-------------------------------------------+----------------------------------+
...
+-------------------------------------------+----------------------------------+
| Kernel                                    | 5.12.19 #mainline-lxos-g1634042087 |
+-------------------------------------------+----------------------------------+
...
+--------------------------------------------+--------+------------------+
| Citrix Appliance                           | 311.0M | Included         |
...
+--------------------------------------------+--------+------------------+
| Cisco WebEx VDI                            |  49.0M | Not included     |
+--------------------------------------------+--------+------------------+
| Cisco Webex Meetings VDI                   |  92.8M | Not included     |
+--------------------------------------------+--------+------------------+
| Zoom Media Plugin                          | 127.5M | Not included     |
+--------------------------------------------+--------+------------------+
...
+--------------------------------------------+--------+------------------+
| HP Factory deployment documentation        |  27.0M | Included         |
+--------------------------------------------+--------+------------------+
...
* Adding smartcard readers while the session is ongoing does not work. The
  ...
  status.
* Citrix has known issues with GStreamer 1.0 which describe problems with
  multimedia redirection of H.264, MPEG1 and MPEG2. GStreamer 1.0 is used if
...
* Keyboard Input Source Language Synchronization works only when using a local
  layout which has deadkeys enabled.  
...
* PCoIP sessions may crash in some cases. Please switch to Blast Protocol
  instead. In case H.264/HEVC encoding would be too demanding for your
  environment, you can switch it off.
* Client drive mapping and USB redirection for storage devices can be switched
  on at the same time but there're problems under some circumstances:  
  Horizon Client tracks the drives which are dynamically mounted and adds them
  to the remote session using client drive mapping, I.e. USB redirection is then
  not used for theses devices.  
  However in case of devices like USB SD card readers, Horizon does not map them
  as client drives but forcefully uses USB-redirection which results in an
  unclean unmount.  
  As a work-around the IDs of these card readers can be added to IGEL USB access
  rules and denied.
...
* With enabled Citrix Browser Content Redirection Firefox has no H.264 and AAC
  multimedia codec support. To get the multimedia support Citrix Browser Content
  Redirection must be disabled. Citrix Browser Content Redirection is disabled
  by default.
...
Release Notes 11.06.240 (Based On 11.06.232)
--------------------------------------------------------------------------------
...
* Use this parameter to enable dead keys:
...
Release Notes 11.06.232 (Based On 11.06.230)
--------------------------------------------------------------------------------
...
* Integrated **WebEx VDI 41.10.0.20213**
...
* Added new parameter that can help to avoid incompatibilities between the AAD
  login and the QWebEngine we use. Enable this new parameter on devices where
  the AAD login hangs (likely showing a white screen after the username has been
  entered)
...
* Fixed bug: Enabling wpa supplicant's background scan was broken (settings
  network.interfaces.wirelesslan.device0.bgscan.**) since the step to Network
  Manager 1.30.0

### Smartcard

* Fixed minor bug in pcsc_scan and ATR_analysis tools and updated list of known
  smartcards.

### Audio

* USB headsets now switch the sound of the applications
